...
function createCompartments(no_compartments) {
    const { comp_x, comp_y, compy_angle, comp_length, comp_width, comp_height } = getCompartmentConfiguration(deck_configuration);

    const obstacleMeshes = [];
    for (let i = 0; i < no_compartments; i++) {
        const compartment = new THREE.Mesh(
            new THREE.BoxGeometry(comp_length[i], comp_width[i], comp_height[i]),
            new THREE.MeshBasicMaterial({ color: 'yellow' })
        );

        compartment.position.set(comp_x[i], comp_y[i], 1);
        compartment.rotation.z = Math.PI * compy_angle[i] / 180.0;

        const compartmentBB = new THREE.Box3().setFromObject(compartment);
    ...
    return { compartments, obstacleMeshes, compartmentsBB };
}
...
function getCompartmentConfiguration(deck_configuration) {
    if (deck_configuration === "simple") {
        return {
...
function handleRadioChange(event) {
    ...
    console.log(`Configuration selected: ${deck_configuration}`);

    // Stop animations and reset buttons
    ...
    removeMeshes(compartmentsMeshes);  
    removeMeshes(persons.map(p => p.geometry)); 
    removeMeshes([deck, mustering, mustering_inner]);
    ...
    // Update deck and mustering station parameters
    if (deck_configuration === "simple") {
        mes_x_global = 105; mes_y_global = 17; mes_width = 10; mes_length = 5; deck_length = 105.2; deck_width = 34;
    } else {
        mes_x_global = 11; mes_y_global = 0; mes_width = 2; mes_length = 2; deck_length = 13; deck_width = 14;
    }

    mes_x = mes_x_global - deck_length / 2;
    mes_y = mes_y_global - deck_width / 2;

    // Recreate scene elements in correct order
    let { deck: newDeck, deckBB: newDeckBB } = createDeck(deck_length, deck_width, 0);
    deck = newDeck; deckBB = newDeckBB;

    let { mustering: newMustering, mustering_inner: newMusteringInner, MusteringBB: newMusteringBB } =
        addMusteringStation(mes_x, mes_y, mes_width, mes_length, 0);
    mustering = newMustering; mustering_inner = newMusteringInner; MusteringBB = newMusteringBB;

    let { compartments: newCompartments, obstacleMeshes: newMeshes, compartmentsBB: newCompartmentsBB } =
        createCompartments(no_compartments);
    compartments = newCompartments; compartmentsMeshes = newMeshes; compartmentsBB = newCompartmentsBB;

    persons = createPerson(no_persons).persons;  // Add persons last

    ShowDeck();  // Refresh scene
}
